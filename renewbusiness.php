...
$sql=$insertb->insertbusiness($_SESSION['id'],$_POST['name'],$_POST['address'],$_POST['size'],$_POST['type'],$_POST['pname'],$_POST['pno'],$_POST['email'],$_POST['bno'],$_POST['state'],$_POST['government'],$_POST['year']);
if($sql){
    $id=$sql;
...
{ 
$sql=$insertb->insertvehicle($_SESSION['id'],$_POST['ownername'],$_POST['registrationno'],$_POST['chasisno'],$_POST['make'],$_POST['manufactureyear'],$_POST['vgovernment'],$_POST['vstate'],$_POST['type'],$_POST['year'],$_POST['address'],$_POST['mobile']);
if($sql){ 
    $id=$sql;
...
{ 
$sql=$insertb->insertproperty($_SESSION['id'],$_POST['title'],$_POST['mobile'],$_POST['address1'],$_POST['address2'],$_POST['pgovernment'],$_POST['pstate'],$_POST['type'],$_POST['year']);
if($sql){
    $id=$sql;
   ...
   <div class="main-panel">
          <div class="content-wrapper">
			...
			<div class="radio-button-choose business-button-choose">
             ...
             <input type="radio" id="vehicle-register" name="new-register" value="vehicle-register" <?php if(isset($_GET['text']) && $_GET['text']=='v'){ echo 'checked'; }?>> <label for="vehicle-register">Vehicle Radio & Parking Permit</label>
				     ...
				     <label class="input-label">Size of Business<sup> *</sup></label>
                      <select id="Business-selector" name="size" required>
					   <option value="" selected>-- Select Business Type --</option>
                        ...
                        <?php }elseif($busdata[0]['size']=='medium'){?>
                        <option value="medium">Medium (Self managed with less than 4 support Staff) </option>
                        <option value="large">Large ( Self managed with 4 staffs and above)</option>
                        <?php }elseif($busdata[0]['size']=='large'){?>
                        <option value="large">Large ( Self managed with 4 staffs and above)</option>
                        <?php } ?>
                     </select>
				   </div>
					  ...
					  <input type="email" class="" name="email" value="<?php echo $busdata[0]['email']?>"  required>
				   	...
				   	<div class="col-md-6">
                     	<div class="input-field"  id="mobilecode">
					  		...
					  		<input type="tel" class="" name="bno"  value="<?php echo $busdata[0]['businessmobile']?>"   required >
					 	</div>
				   	</div> 
				<div class="col-md-6">
				     <div class="input-field">
        ...
        <option></option>
			 ...
			 <div class="vehicle-register business-new <?php if(isset($_GET['text']) && $_GET['text']=='v'){ echo 'show" style="display: block;"'; }?>">
					<?php  
							$vecdata=$backdata->getwheredata('vehicle', 'id', $_GET['id']);
					?>
                <form class="business-form"  method="post" action="" enctype="multipart/form-data">
			     <div class="row">
				   <div class="col-md-12">
				   ...
				   <div class="col-md-6">
				    ...
				    <div class="col-md-6">
				     <div class="input-field"   id="mobilecode">
				  		<label class="input-label">Mobile( xxxxxxxxxx) <sup> *</sup></label><input type="tel" class="" name="mobile"  required >
					 </div>
				   </div>
				   <div class="col-md-6">
				     <div class="input-field">
				  ...
				  <div class="col-md-6">
				     <div class="input-field">
					  ...
					  <select name="type" required>
                                            <?php if($vecdata[0]['usetype']=='Private'){ ?> 
					   ...
					   <?php }elseif($vecdata[0]['usetype']=='INTRA - STATE COMMERCIAL'){?>
                         <option value="INTRA - STATE COMMERCIAL">INTRA - STATE COMMERCIAL (N 3000)</option>
					   <option value="INTER - STATE COMMERCIAL">INTER - STATE COMMERCIAL (N 5000)</option>
					   <option value="Tricycle(KEKE NAPEP or KEKE MARWA)">Tricycle (KEKE NAPEP or KEKE MARWA)(N500)</option>
					   <option value="Vehicles other than Mini Buses and Vans">Vehicles other than Mini Buses and Vans e.g Trucks, Luxury Buses, etc (N2000)</option>
					   <?php }elseif($vecdata[0]['usetype']=='INTER - STATE COMMERCIAL'){?>
					   <option value="INTER - STATE COMMERCIAL">INTER - STATE COMMERCIAL (N 5000)</option>
					   <option value="Tricycle(KEKE NAPEP or KEKE MARWA)">Tricycle (KEKE NAPEP or KEKE MARWA)(N500)</option>
					   <option value="Vehicles other than Mini Buses and Vans">Vehicles other than Mini Buses and Vans e.g Trucks, Luxury Buses, etc (N2000)</option>
					    <?php }elseif($vecdata[0]['usetype']=='Tricycle(KEKE NAPEP or KEKE MARWA)'){?>
					   <option value="Tricycle(KEKE NAPEP or KEKE MARWA)">Tricycle (KEKE NAPEP or KEKE MARWA)(N500)</option>
					   <option value="Vehicles other than Mini Buses and Vans">Vehicles other than Mini Buses and Vans e.g Trucks, Luxury Buses, etc (N2000)</option>
					        <?php }elseif($vecdata[0]['usetype']=='Vehicles other than Mini Buses and Vans'){?>
					   <option value="Vehicles other than Mini Buses and Vans">Vehicles other than Mini Buses and Vans e.g Trucks, Luxury Buses, etc (N2000)</option>
					     <?php } ?> 
			 ...
			 <div class="property-register business-new <?php if(isset($_GET['text']) && $_GET['text']=='p'){ echo 'show" style="display: block;"'; }?>">
					<?php  	
							$prodata=$backdata->getwheredata('property', 'id', $_GET['id']);
                    ?>
			     ...
			     <div class="row">
				   ...
				   <div class="col-md-6">
				     ...
				     <label class="input-label"> Type of Property<sup> *</sup></label>
                        <select name="type" required>
          <?php if($prodata[0]['propertytype']=='small'){ ?> 
			...
			<?php }elseif($prodata[0]['propertytype']=='medium'){?>
                  			<option value="medium">Medium(Bungalow of not more than 3 bedrooms, storey building of not more than 4 units, Duplexes of not more than 4 bedrooms)</option>
						<option value="large">Large(Duplexes of more than 4 bedrooms, storey buildings of more than 4 units, Plazas, Warehouse)</option>
						<option value="multipurpose">Multipurpose: (Hotels, Owned school premises, Event Halls & Tent, Resort and Relaxation centers, Hospital, Ultra Mart, Medical Centres etc.)</option>
			<?php }elseif($prodata[0]['propertytype']=='large'){?>
                      				<option value="large">Large(Duplexes of more than 4 bedrooms, storey buildings of more than 4 units, Plazas, Warehouse)</option>
						<option value="multipurpose">Multipurpose: (Hotels, Owned school premises, Event Halls & Tent, Resort and Relaxation centers, Hospital, Ultra Mart, Medical Centres etc.)</option>
			<?php }elseif($prodata[0]['propertytype']=='multipurpose'){?>
                                                <option value="multipurpose">Multipurpose: (Hotels, Owned school premises, Event Halls & Tent, Resort and Relaxation centers, Hospital, Ultra Mart, Medical Centres etc.)</option>
			<?php }?>
